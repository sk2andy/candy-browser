...
| Android new tab | `BrowserController`, `NewTabPage`, `NewTabFavoriteGrid` | Hide the favorites surface when the collection is empty or browsing is private. Show favorites as a four-column favicon grid with a neutral title-initial fallback, including blank-tab previews. Size the surface for at most five rows (20 entries), shrink it to the available height, and scroll further entries inside the surface rather than scrolling the whole new-tab hero. |
