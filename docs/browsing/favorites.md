...
| Android new tab | `BrowserController`, `NewTabPage`, `NewTabFavoriteGrid` | Hide the favorites surface when the collection is empty or browsing is private. Present its localized Favorites heading above a low-contrast outlined shelf, then show a generously inset four-column favicon grid with centered two-line labels and a neutral title-initial fallback, including blank-tab previews. Crop each favicon edge-to-edge into a shuffled Material shape and morph it through a compact 20-shape cycle, excluding thin elongated shapes. Assign a distinct starting shape to every entry in the five visible rows and freeze the current contour into the launch motion. Size the shelf for at most five rows (20 entries), shrink it to the available height, and scroll further entries inside the shelf rather than scrolling the whole new-tab hero. Returning through the tab overview releases the non-interactive preview after the blank-tab content frame instead of waiting for a browser-engine frame that blank tabs never produce. |
| Favorite motion | `FavoriteAnimationSpeed`, `NewTabFavoriteLaunchOverlay`, `BrowserSettingsPage`, `BrowserSessionStore` | By default, let new-tab favicon shapes morph at the Normal speed with short rests, then animate a selected favorite through a short Candy-colored arc into the search hero before opening it. The Browser settings provide Relaxed, Normal, and Fast morph speeds; the animation switch disables both motions and restores immediate navigation. |
