...
| Search | `BrowsingFavoritesRules`, `FavoritesScreen`, `BrowserFavoritesRules`, `BrowserFavoritesSurface` | Filter locally and case-insensitively. Android searches stored title, URL and host, bounds input to 256 characters, discards invalid persisted URLs and collapses canonical duplicates. iOS searches its derived host title and URL. |
