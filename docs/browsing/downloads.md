...
- Gecko-generated `blob:` responses can carry status `0` because they have no HTTP response. Candy accepts that status only for `blob:` transfers with a response body; HTTP responses and other blob status errors keep the normal success-status requirement.
