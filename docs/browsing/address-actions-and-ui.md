...
| Main-menu morph | `shared/ui/AddressMenuMorphRules.kt`, `shared/ui/BrowserMainMenu.kt`, `ui/BrowserBottomBar.kt` | Opening **More** expands the menu from its 48 dp trailing action anchor while the address surface scales and its content fades. Dismissal reverses the same 160 ms transition before removing the popup. |
