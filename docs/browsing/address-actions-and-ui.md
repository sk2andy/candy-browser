...
| Automatic right parking | `AddressBarAutoDockRules`, `TextInputOcclusionScript`, Android engine session ports | After a successful main-frame load, same-document URL navigation, or webpage-owned IME opening, the selected regular tab gets one bounded probe after layout settles. Visible webpage IME and address-bar geometry updates debounce that probe through the keyboard transition; handing an already-visible IME back from browser chrome schedules it too. Chrome-owned address editing and find-in-page are excluded. The address-bar rectangle is normalized against the renderer's visible height above the IME and must overlap a visible text-like `input`, `textarea`, or `contenteditable` editor while the top-level document has no remaining downward scroll. GeckoView routes the fixed probe through Candy's internal extension; System WebView evaluates the same fixed predicate. No DOM mutation or scroll observer is installed. Tab, engine session, navigation generation, final URL, chrome geometry and docking availability must remain current before applying the result. Private tabs are excluded because parking persists globally. A successful result uses normal right parking and remains parked until the user restores the address bar. |
